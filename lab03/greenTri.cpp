...
const char* fragmentSource = R"(
   #version 330
    ...
    bool isDaytime(vec2 mercator) {
        ...
        float lonRad = (mercator.x - 0.5) * 2.0 * PI;
        
      
        float latRad = atan(sinh((mercator.y - 0.5) * 2.0 * PI));
...
const float AXIS_TILT = 23.0f * PI / 180.0f;
...
vec2 MercatorToSpherical(float u, float v) {
    ...
    float mercatorY = (v - 0.5f) * 2.0f * PI;
    float latitude = atan(sinh(mercatorY));
    
    
    if (latitude > 85.0f * PI / 180.0f) latitude = 85.0f * PI / 180.0f;
    if (latitude < -85.0f * PI / 180.0f) latitude = -85.0f * PI / 180.0f;
...
vec2 SphericalToMercator(float lon, float lat) {
    ...
    float mercatorY = log(tan(PI/4.0f + lat/2.0f));
    
    
    float v = 0.5f + mercatorY / (2.0f * log(tan(PI/4.0f + MAX_LAT/2.0f)));
...
float CalculateDistance(vec3 p1, vec3 p2) {
    ...
    float distanceKm = angle * EARTH_CIRCUMFERENCE / (2.0f * PI);
